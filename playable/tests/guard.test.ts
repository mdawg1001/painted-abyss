...
test('guard starts each life somewhere new on the perimeter, away from the player',()=>{
 ...
 for(let life=0;life<12;life++){
  ...
  assert.equal(m.guard.waypoint,(i+1)%route.length,'heads on to the next stop');
 }
...
test('patrol loops the perimeter: walks straight between stops, stops and looks at each',()=>{
 ...
 let maxDev=0;
 for(let i=0;i<Math.round(14*60/dt);i++){
  m.breathWaterY=FLOOR_Y-.1; // hold the leak back so he can walk the whole loop
  m.update(dt,false);
  const g=m.guard;
  ...
  if(mv>1e-5){
   ...
   // Straight: stays on the line from the previous stop to the next.
   const b=route[g.waypoint],a=route[(g.waypoint-1+route.length)%route.length];
 ...
 assert.ok(visited>=route.length,`completes the loop (${visited} stops of ${route.length})`);
 assert.ok(maxDev<.05,`straight lines between stops (max ${maxDev.toFixed(3)} m off)`);
 assert.ok(scanTurn>route.length*1.5,'sweeps his gaze at the stops');
});
...
test('guard sees what is in front of him, hears running, and feels someone right beside him',()=>{
 const setup=(dx:number,dz:number,opts:{torch?:boolean;sprint?:boolean}={})=>{
  const m=new Mission(true);m.breathWaterY=FLOOR_Y-.1;
...
test('after losing the player he rejoins the loop at the nearest stop',()=>{
 ...
 const dMin=Math.min(...route.map(q=>Math.hypot(q.x-10,q.z+60)));
 assert.ok(Math.abs(Math.hypot(w.x-10,w.z+60)-dMin)<1e-9);
