...
export class OceanWorld {
  ...
  material(color:THREE.ColorRepresentation,detail:'rock'|'skin'|'sand'|'plain'='plain',roughness=.82,causticGain=1,maps?:PbrMaps,mossMaps?:PbrMaps){
    ...
    const mossAmount=detail==='rock'?'0.4':detail==='sand'?'0.22':'0.';
    m.onBeforeCompile=shader=>{
      ...
      if(usePbr){
        ...
        detailCode=`{
          ...
          float moss=min(mossCoverage(vOceanWorld,wn,arm.r,${mossAmount}),.20);
    ...
    m.customProgramCacheKey=()=>`${detail}:${gain.toFixed(2)}:pbr${usePbr?maps!.key:'0'}:moss${useMoss?mossMaps!.key+':20pct':'0'}`;
    return m;
  }
