...
function App(){
 ...
 return <main className={playing?'app playing':'app'}>
  ...
  {playing&&m&&<>
   ...
   <div className="inventory"><div className="inventory-label">{m.pending!==null?'INVENTORY FULL · SELECT AN ITEM TO LEAVE BEHIND':'SELECT 1–5 · R USES · CONSUMED'}<span>SLOT {m.selected+1}/5</span></div><div className="slots">{m.inventory.map((item,i)=>{const selected=i===m.selected;const pulse=selected&&m.feedbackKind?m.feedbackKind:'';return <div className={`slot ${selected?'selected':''} ${item==='relic'?'relic':''} ${pulse?`pulse-${pulse}`:''}`} key={selected?`${i}-p${m.feedbackPulse}`:i}><kbd>{i+1}</kbd><Icon item={item}/><span>{item?ITEMS[item].short:'Empty'}</span>{selected&&<em className="slot-mark">SELECTED</em>}</div>;})}</div><div className="item-note">{m.inventory[m.selected]?`${ITEMS[m.inventory[m.selected]!].description}`:'Empty slot'}</div></div>
   <footer><span><kbd>W A S D</kbd> swim <kbd>Space / Q</kbd> up / down <kbd>Shift</kbd> sprint</span><span><kbd>1–5</kbd> select slot <kbd>R</kbd> use / consume <kbd>G</kbd> drop <kbd>Esc</kbd> pause</span></footer>
